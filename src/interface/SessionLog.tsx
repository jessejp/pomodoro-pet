...
const SessionLog = () => {
  const [foldWindow, setFoldWindow] = useState(false);
  ...
  return (
    <Draggable insetTailwindCss="bottom-[10%] right-12">
      <div className="z-10 w-96 rounded border-4 border-violet-700 bg-orangeFlavour px-4 py-4">
        <button
          className="pointer-events absolute left-5 top-5 text-2xl hover:scale-125"
          onClick={() => setFoldWindow((state) => !state)}
        >
          {foldWindow ? "👇" : "👆"}
        </button>
          ...
          <>
            <div className="flex h-fit max-h-80 flex-col gap-1 overflow-y-scroll rounded bg-white p-2">
              ...
              {sessionLog.map((entry, index) => (
                <div
                  key={index}
                  className="flex py-1 text-left odd:bg-neutral-100"
                >
                  <div className="w-14">
                    {!showTimeWithBreaks &&
                    ...
                    {!!showTimeWithBreaks &&
                      `${
                        Math.round((entry.minutesWithBreaks / 60) * 100) / 100
                      } h`}
                  </div>
            ...
            <div className="flex flex-row-reverse gap-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setShowTimeWithBreaks(event.target.checked)
                }
              />
            </div>
          </>
